                    ...
                    if(getItem(position).clickListener != null){
                        if (position==0) {
                            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                                if (context.getApplicationContext().checkSelfPermission(Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                                    Toast.makeText(context, "没有照相权限", Toast.LENGTH_SHORT).show();
                                    return;
                                }
                            }
                        }
