package com.hyzs.onekeyhelp.lifehelp.bean;

/**
 * Created by hyzs123 on 2017/3/20.
 */

public class HelpOtherDetailsBean {



    private LifeSeekMyHelpDetailsBean lifeSeekMyHelpDetails;
    private String message;
    private String code;

    public LifeSeekMyHelpDetailsBean getLifeSeekMyHelpDetails() {
        return lifeSeekMyHelpDetails;
    }

    public void setLifeSeekMyHelpDetails(LifeSeekMyHelpDetailsBean lifeSeekMyHelpDetails) {
        this.lifeSeekMyHelpDetails = lifeSeekMyHelpDetails;
    }

    public String getMessage() {
        return message;
    }

    public void setMessage(String message) {
        this.message = message;
    }

    public String getCode() {
        return code;
    }

    public void setCode(String code) {
        this.code = code;
    }

    public static class LifeSeekMyHelpDetailsBean {

        private int Seek_ID;
        private int Seek_UserID;
        private String Seek_Text;
        private String Seek_Voice;
        private String Seek_AffixImgList;
        private String Seek_Type_b;
        private int Seek_State;
        private String HelpCount;
        private String CommunityName;
        private String GrassrootsHero;
        private String uesrname;
        private String firstname;
        private String face;
        private int Seek_Type;
        private long Seek_Time;
        private int NCCS;
        private int RNA;
        private int Seek_Money;
        private int HelpID;
        private int Help_Money;
        private int Help_Integral;
        private int Help_Type;

        public int getSeek_ID() {
            return Seek_ID;
        }

        public void setSeek_ID(int Seek_ID) {
            this.Seek_ID = Seek_ID;
        }

        public int getSeek_UserID() {
            return Seek_UserID;
        }

        public void setSeek_UserID(int Seek_UserID) {
            this.Seek_UserID = Seek_UserID;
        }

        public String getSeek_Text() {
            return Seek_Text;
        }

        public void setSeek_Text(String Seek_Text) {
            this.Seek_Text = Seek_Text;
        }

        public String getSeek_Voice() {
            return Seek_Voice;
        }

        public void setSeek_Voice(String Seek_Voice) {
            this.Seek_Voice = Seek_Voice;
        }

        public String getSeek_AffixImgList() {
            return Seek_AffixImgList;
        }

        public void setSeek_AffixImgList(String Seek_AffixImgList) {
            this.Seek_AffixImgList = Seek_AffixImgList;
        }

        public String getSeek_Type_b() {
            return Seek_Type_b;
        }

        public void setSeek_Type_b(String Seek_Type_b) {
            this.Seek_Type_b = Seek_Type_b;
        }

        public int getSeek_State() {
            return Seek_State;
        }

        public void setSeek_State(int Seek_State) {
            this.Seek_State = Seek_State;
        }

        public String getHelpCount() {
            return HelpCount;
        }

        public void setHelpCount(String HelpCount) {
            this.HelpCount = HelpCount;
        }

        public String getCommunityName() {
            return CommunityName;
        }

        public void setCommunityName(String CommunityName) {
            this.CommunityName = CommunityName;
        }

        public String getGrassrootsHero() {
            return GrassrootsHero;
        }

        public void setGrassrootsHero(String GrassrootsHero) {
            this.GrassrootsHero = GrassrootsHero;
        }

        public String getUesrname() {
            return uesrname;
        }

        public void setUesrname(String uesrname) {
            this.uesrname = uesrname;
        }

        public String getFirstname() {
            return firstname;
        }

        public void setFirstname(String firstname) {
            this.firstname = firstname;
        }

        public String getFace() {
            return face;
        }

        public void setFace(String face) {
            this.face = face;
        }

        public int getSeek_Type() {
            return Seek_Type;
        }

        public void setSeek_Type(int Seek_Type) {
            this.Seek_Type = Seek_Type;
        }

        public long getSeek_Time() {
            return Seek_Time;
        }

        public void setSeek_Time(long Seek_Time) {
            this.Seek_Time = Seek_Time;
        }

        public int getNCCS() {
            return NCCS;
        }

        public void setNCCS(int NCCS) {
            this.NCCS = NCCS;
        }

        public int getRNA() {
            return RNA;
        }

        public void setRNA(int RNA) {
            this.RNA = RNA;
        }

        public int getSeek_Money() {
            return Seek_Money;
        }

        public void setSeek_Money(int Seek_Money) {
            this.Seek_Money = Seek_Money;
        }

        public int getHelpID() {
            return HelpID;
        }

        public void setHelpID(int HelpID) {
            this.HelpID = HelpID;
        }

        public int getHelp_Money() {
            return Help_Money;
        }

        public void setHelp_Money(int Help_Money) {
            this.Help_Money = Help_Money;
        }

        public int getHelp_Integral() {
            return Help_Integral;
        }

        public void setHelp_Integral(int Help_Integral) {
            this.Help_Integral = Help_Integral;
        }

        public int getHelp_Type() {
            return Help_Type;
        }

        public void setHelp_Type(int Help_Type) {
            this.Help_Type = Help_Type;
        }

}}
