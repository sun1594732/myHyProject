...
public class MineAttentionForOtherFragment extends Fragment{
    private ListView listView;
    private MineAttentionForMeAdapter adapter;
    private MineAttentionBean bean;
    private Dialog dialog;
    private List<MineAttentionBean.UserInfoBean> list = new ArrayList<>();
    ...
    private void requestData() {
        String url = PortUtil.WatchMylist;
        ...
        NetRequest.ParamPostRequest(url, map, new NetRequest.getDataCallback() {
            ...
            public void getData(String data) {
                try {
                    bean = new Gson().fromJson(UrlDecodeUtil.urlCode(data), MineAttentionBean.class);
                    initList();
                } catch (Exception e) {
    ...
    private void initList() {
        ...
        if (loadFlag) {
            loadFlag = false;
            if (bean.getUserInfo().size() == 0) {
                Toast.makeText(getActivity(), "没有更多数据", Toast.LENGTH_SHORT).show();
                return;
            }
            list.addAll(bean.getUserInfo());
            adapter.notifyDataSetChanged();
        } else {
            list.clear();
            list.addAll(bean.getUserInfo());
            if (null == adapter) {
                adapter = new MineAttentionForMeAdapter(getActivity(), list);
